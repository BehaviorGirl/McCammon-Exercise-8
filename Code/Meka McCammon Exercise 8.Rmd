...
```
```{r echo=FALSE}
likert.score(personality, 6, c(3,4,8))
```
```{r include=FALSE}
total <- likert.score(personality, 6, c(3,4,8))
```
```{r echo=FALSE}
personality.total <- cbind(personality,total)
personality.total
```
```{r echo=TRUE}
write.csv(personality.total,
          file = "New Personality (with total score).csv",
          row.names = FALSE)
```
